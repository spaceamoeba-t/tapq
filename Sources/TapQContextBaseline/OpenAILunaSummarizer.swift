...
/// Cloud spoken summarization using OpenAI's Responses API and GPT-5.6 Luna.
///
/// Provider failures return `nil` so the summarizer chain can fail open through its
/// deterministic local fallback. The API key and submitted reply are never logged.
public struct OpenAILunaSummarizer: SpokenSummarizing {
    ...
    private func makeRequest(text: String) throws -> URLRequest {
        let body: [String: Any] = [
            "model": model,
            "instructions": CloudSpokenSummaryContract.instructions,
            "input": "Summarize this reply for speech:\n\n\(text)",
    ...
    private static func decodeResponse(_ data: Data) -> SpokenSummary? {
        ...
        return CloudSpokenSummaryContract.decode(text)
    }
